...
class BlockA(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, dilation=1, norm_layer=None, start_with_relu=False):
...
class Enc(nn.Module):
    def __init__(self, in_channels, out_channels, blocks, norm_layer=None):
        ...
        block = list()
        block.append(BlockA(in_channels, out_channels, 2, norm_layer=nn.BatchNorm2d))
        for i in range(blocks - 1):
            block.append(BlockA(out_channels, out_channels, 1, norm_layer=nn.BatchNorm2d))
...
def get_xception_a(pretrained=False, root='../models', **kwargs):
