...
class DFANet(nn.Module):
    ...
    def forward(self, x):
        ...
        ## DS1开始
        # print('stage2_out尺寸：', stage2_out.shape)
        DS1 = F.interpolate(self.DS1_(stage2_out), scale_factor=8, mode='bilinear', align_corners=True)
        DS1_out = list()
        DS1_out.append(DS1)
        ###DS1结束
        ...
        stage3_enc2_decoder = F.interpolate(self.enc2_3_reduce(stage3_enc2), scale_factor=4,
                                            mode='bilinear', align_corners=True)
        #### DS2开始
        # print('stage3_enc2_decoder尺寸：', stage3_enc2_decoder.shape)
        DS2 = F.interpolate(self.conv_out(stage3_enc2_decoder), scale_factor=4,
                            mode='bilinear', align_corners=True)
        DS2_out = list()
        DS2_out.append(DS2)
        ####DS2结束
        ...
        #### DS3开始
        # print('fusion1尺寸：', fusion.shape)
        DS3 = F.interpolate(self.conv_out(fusion), scale_factor=4,
                            mode='bilinear', align_corners=True)
        DS3_out = list()
        DS3_out.append(DS3)
        ### DS3结束
        ...
        #### DS4
        # print('fusion2尺寸：',fusion.shape)

        outputs = list()
        out = self.conv_out(fusion)
        # print('out1尺寸：', out.shape)
...
def get_dfanet(dataset='citys', backbone='resnet50', pretrained=False, root='~/.torch/models',
    ...
    if pretrained:
        ...
        #                       map_location=device))
        model.load_state_dict(torch.load('../models/dfanet_resnet50_citys_158.pth'))
    return model
