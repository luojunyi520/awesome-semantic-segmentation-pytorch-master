...
def parse_args():
    ...
    # model and dataset
    parser.add_argument('--model', type=str, default='dfanet',
...
class Trainer(object):
    ...
    def train(self):
        ...
        # summary(self.model, input_size=[(3, 320, 320)], batch_size=2, device="cuda")
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            # train_pixAcc_save = pixAcc + train_pixAcc_save
            #
            # if iteration % save_per_iters == 0 and save_to_disk:
            #     file = open("train_pixAcc.txt", encoding="utf-8", mode="a")
            #     train_pixAcc = train_pixAcc_save / save_per_iters
            #     file.write(str(train_pixAcc) + '\n')
            #     file.close()
            #     train_pixAcc_save = 0

            if iteration % log_per_iters == 0 and save_to_disk:
            # if iteration % 1 == 0 and save_to_disk:
                logger.info(
            ...
            if iteration % save_per_iters == 0 and save_to_disk:
                save_checkpoint(self.model, self.args, is_best=False, iter=iteration)

            if not self.args.skip_val and iteration % val_per_iters == 0:
                self.validation()
                self.model.train()

        save_checkpoint(self.model, self.args,  is_best=False, iter='')
    ...
    def validation(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            with torch.no_grad():
                outputs = model(image)
            output0,output1,output2,output3=tuple(outputs)


            self.metric.update(output0, target)


            pixAcc, mIoU = self.metric.get()
            logger.info("Sample: {:d}, Validation pixAcc: {:.3f}, mIoU: {:.3f}".format(i + 1, pixAcc/2, mIoU/2))

            val_pixAcc_save = pixAcc + val_pixAcc_save

            ####计算val损失
            val_loss_dict = self.criterion(*outputs, target)
            ...
            if i + 1 == val_samples:
                ...
                file = open("val_pixAcc.txt", encoding="utf-8", mode="a")
                val_pixAcc = val_pixAcc_save / val_samples
                file.write(str(val_pixAcc) + '\n')
                file.close()
...
def save_checkpoint(model, args,  is_best=False, iter=''):
    """Save Checkpoint"""
    directory = os.path.expanduser(args.save_dir)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = '{}_{}_{}_{}.pth'.format(args.model, args.backbone, args.dataset, iter)
    ...
    if is_best:
        iter = ''
        best_filename = '{}_{}_{}_{}_best_model.pth'.format(args.model, args.backbone, args.dataset, iter)
